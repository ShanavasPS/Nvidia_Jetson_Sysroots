...
SYSROOT_ARCHIVE_FILE=sysroot-nvidia_jetson_xavier_ubuntu_18.04.6_aarch64.tar.xz

# Nvidia defines
SOC_NAME=t194
NVIDIA_DOWNLOAD_LOCATION=https://developer.nvidia.com/embedded/l4t/r32_release_v7.1/t186
L4T_DRIVER_PACKAGE=jetson_linux_r32.7.1_aarch64.tbz2
SAMPLE_ROOT_FILESYSTEM=tegra_linux_sample-root-filesystem_r32.7.1_aarch64.tbz2
...
# Patch to fix the blueman error
sudo sed -i "s/invoke-rc.d dbus reload/#invoke-rc.d dbus reload/" $SYSROOT_DIRECTORY/var/lib/dpkg/info/blueman.postinst
...
echo "/usr/lib/aarch64-linux-gnu/tegra" >> $SYSROOT_DIRECTORY/etc/ld.so.conf.d/cuda-10-2.conf
